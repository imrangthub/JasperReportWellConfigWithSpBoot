...
import com.imran.report.JasperExportFormat;
...
public class JasperService {
    ...
    private JasperPrint generatePrinter(CusJasperReportDef reportDef) throws JRException, IOException {
        ...
        if (jrDataSource != null) {
            if (resource.getFilename().endsWith(".jasper")) {
                ...
                jasperPrint = JasperFillManager.fillReport(JasperCompileManager.compileReport(resource.getInputStream()), reportDef.getParameters(), jrDataSource);
            }
        } 
     ...
     */
    private
    static ExporterOutput getExporterOutput(JasperExportFormat format, ByteArrayOutputStream byteArrayOutputStream) throws Exception {
        switch (format) {
        //common cases
            case PDF_FORMAT:
            case XLS_FORMAT:
        ...
        //specific cases
            ...
            case CSV_FORMAT: return new SimpleWriterExporterOutput(byteArrayOutputStream);
            case RTF_FORMAT: return new SimpleWriterExporterOutput(byteArrayOutputStream);
   ...
   /*     if (reportDef.getUseDefaultConfiguration()) {
        ...
        if (!reportDef.getReportConfiguration().isEmpty()) {
            Field[] fields = getExporterFields(reportDef.getReportFormat());
            applyCustomConfiguration(fields, exporter, reportDef.getReportConfiguration());
        }*/
        return exporter;
    }
    ...
    private static Field[] getExporterFields(JasperExportFormat format) {
        ...
        case PDF_FORMAT: return PdfExporterConfiguration.class.getFields();
        case HTML_FORMAT: return HtmlExporterConfiguration.class.getFields();
        case CSV_FORMAT: return CsvExporterConfiguration.class.getFields();
        case XLS_FORMAT: return XlsExporterConfiguration.class.getFields();
        case XLSX_FORMAT: return XlsxExporterConfiguration.class.getFields();
        case RTF_FORMAT: return RtfExporterConfiguration.class.getFields();
        case DOCX_FORMAT: return DocxExporterConfiguration.class.getFields();
